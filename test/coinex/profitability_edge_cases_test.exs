  ...
  describe "Profitability Edge Cases" do
    test "minimum market order with 0.001% TP limit order results in loss (0.001% << 0.08% break-even)" do
      ...
      balance = %Balance{
        ...
        # Frozen for minimum 0.0001 BTC order
        frozen: Decimal.new("5"),
      ...
      {balance_after_buy, positions_after_buy} =
      ...
      {balance_after_tp, positions_after_tp} =
        ActionCalculator.calculate_action_effect(
          balance_after_buy,
          positions_after_buy,
    ...
    test "break-even calculation: minimum profit needed to overcome fees" do
      ...
      # 0.0001 BTC * $50,000
      order_value = Decimal.new("5")

      # Total fees: taker fee + maker fee
      # 0.0025
      taker_fee = Decimal.mult(order_value, Decimal.new("0.0005"))
      ...
      # Should be > 0.08%
      assert Decimal.gt?(profit_percentage, Decimal.new("0.0008"))
      # Should be < 0.09%
      assert Decimal.lt?(profit_percentage, Decimal.new("0.0009"))
      ...
      # > 0.08%
      assert Decimal.gt?(break_even_percentage, Decimal.new("0.0800"))
      # < 0.0802%
      assert Decimal.lt?(break_even_percentage, Decimal.new("0.0802"))
    end
    ...
    test "fee threshold: orders below 0.08% profit margin are unprofitable (corrected)" do
      ...
      test_cases = [
        # 0.001% - far below break-even
        {Decimal.new("0.00001"), "0.001%", false},
        # 0.01% - still far below
        {Decimal.new("0.0001"), "0.01%", false},
        # 0.05% - getting closer but unprofitable
        {Decimal.new("0.0005"), "0.05%", false},
        # 0.07% - below break-even threshold
        {Decimal.new("0.0007"), "0.07%", false},
        # 0.08% - at break-even (tiny loss due to precision)
        {Decimal.new("0.0008"), "0.08%", false},
        # 0.09% - barely profitable! (~0.01% net profit)
        {Decimal.new("0.0009"), "0.09%", true},
        # 0.1% - profitable! (~0.02% net profit)
        {Decimal.new("0.001"), "0.1%", true},
        # 0.15% - clearly profitable
        {Decimal.new("0.0015"), "0.15%", true}
      ]
    ...
    test "0.1% profit target on minimum amount yields 0.02% net profit" do
      ...
      net_profit_percentage =
        Decimal.mult(Decimal.div(expected_net_profit, entry_value), Decimal.new("100"))
      ...
      fee_percent_approx =
        Decimal.mult(Decimal.div(expected_total_fees, entry_value), Decimal.new("100"))
      ...
      assert Decimal.compare(
               Decimal.abs(Decimal.sub(net_profit_percentage, calculated_net_percent)),
               Decimal.new("0.001")
             ) == :lt,
    ...
    test "granular break-even analysis: precise profit margins around 0.08% threshold" do
      ...
      precise_test_cases = [
        # Right at break-even (tiny loss)
        {Decimal.new("0.0008"), "0.080%", false},
        # Just above break-even (tiny profit)
        {Decimal.new("0.000801"), "0.0801%", true},
        # Slightly above break-even  
        {Decimal.new("0.000805"), "0.0805%", true},
        # Clearly above break-even
        {Decimal.new("0.00081"), "0.081%", true},
        # Comfortably profitable
        {Decimal.new("0.00085"), "0.085%", true},
        # 0.09% → ~0.01% net profit (validated above)
        {Decimal.new("0.0009"), "0.090%", true},
        # More profitable
        {Decimal.new("0.00095"), "0.095%", true},
        # 0.1% → ~0.02% net profit
        {Decimal.new("0.001"), "0.100%", true}
      ]
      ...
      Enum.each(precise_test_cases, fn {profit_factor, description, should_be_profitable} ->
        ...
        net_profit_percentage =
          Decimal.mult(Decimal.div(net_pnl, entry_value), Decimal.new("100"))
    ...
    test "high-frequency scenario: multiple small profitable trades vs single large trade" do
      ...
      # 0.2%
      small_trade_profit_factor = Decimal.new("0.002")
      num_small_trades = 10

      {total_small_fees, total_small_gross_profit} =
        Enum.reduce(1..num_small_trades, {Decimal.new("0"), Decimal.new("0")}, fn _i,
                                                                                  {acc_fees,
                                                                                   acc_profit} ->
          entry_value = Decimal.mult(small_trade_amount, Decimal.new("50000"))

          exit_value =
            Decimal.mult(entry_value, Decimal.add(Decimal.new("1"), small_trade_profit_factor))
      ...
      # 0.001 BTC
      large_trade_amount = Decimal.mult(small_trade_amount, Decimal.new("10"))
      # 50 USDT
      large_entry_value = Decimal.mult(large_trade_amount, Decimal.new("50000"))

      large_exit_value =
        Decimal.mult(large_entry_value, Decimal.add(Decimal.new("1"), small_trade_profit_factor))
    ...
    test "margin call scenario: position becomes unprofitable due to fees" do
      ...
      emergency_exit_order = %Order{
        ...
        price: Decimal.new("50100"),
        frozen_amount: Decimal.new("0")
      }

      fill_price = Decimal.new("50100")

      {balance_after_exit, positions_after_exit} =
        ActionCalculator.calculate_action_effect(
          balance,
          positions,
