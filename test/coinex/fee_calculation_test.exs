  ...
  describe "CoinEx Fee Calculation Tests" do
    test "market order applies taker fee (0.05%)" do
      balance = %Balance{
        ...
        frozen: Decimal.new("500"),  # Already frozen when order was placed
      ...
      expected_available = Decimal.new("9999.75")  # 10000 + 500 (unfreeze) - 0.25 (fee) - 500 (margin)
      ...
      assert Decimal.equal?(new_balance.margin_used, Decimal.new("500"))  # New position margin
    ...
    test "limit order applies maker fee (0.03%)" do
      ...
      expected_available = Decimal.new("9999.85")  # 10000 + 500 (unfreeze) - 0.15 (fee) - 500 (margin)
    ...
    test "sell order reducing position still pays fees" do
      balance = %Balance{
        ...
        frozen: Decimal.new("0"),  # No frozen for position-reducing order
      ...
      sell_order = %Order{
        ...
        side: "sell",
        type: "market",
        amount: Decimal.new("0.01"),  # Close entire position
        price: Decimal.new("51000"),  # Exit at higher price
    ...
    test "cumulative fee tracking across multiple trades" do
      ...
      {balance_after_second, _} = ActionCalculator.calculate_action_effect(
        balance_after_first,
    ...
    test "fee calculation is purely transactional (order-independent)" do
      ...
      balance2 = %Balance{
        ...
        total: Decimal.new("10000"),
        unrealized_pnl: Decimal.new("0"),
        total_fees_paid: Decimal.new("50")  # Different starting fees
      }
      
      # Fill same order with different starting states
      {result1, _} = ActionCalculator.calculate_action_effect(
        balance1, positions, {:fill_order, order, Decimal.new("45000")}
      )
      
      {result2, _} = ActionCalculator.calculate_action_effect(
        balance2, positions, {:fill_order, order, Decimal.new("45000")}
      )
      ...
      fee2_increment = Decimal.sub(result2.total_fees_paid || Decimal.new("0"), balance2.total_fees_paid || Decimal.new("0"))
